...
TrainNuNet <- function (inputs,targets,Percent.To.Test) 
{
  ...
  # mean.inputs <- apply(inputs, 2, mean); std.inputs <- apply(inputs, 2, sd); inputs.normalized <- scale(inputs, center = mean.inputs, scale = std.inputs)
  # mean.targets <- mean(targets); std.targets <- sd(targets); targets.normalized <- scale(targets, center = mean.targets, scale = std.targets)
  # 
  ...
  # NN.fitted.Train <- model$fitted.values*std.targets 
  # NN.fitted.Test <- model$fittedTestValues*std.targets 
